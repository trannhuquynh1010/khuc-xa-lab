...
import { OPTICS_QUEST_MAX_ENERGY, type OpticsQuestSnapshot } from "@/lib/optics-quest";
...
export default function OpticsQuestDashboard({ className, schoolYear, presentation = false }: { className: string; schoolYear: string; presentation?: boolean }) {
  ...
  return (
    <section className={`optics-quest-dashboard ${presentation ? "presentation" : ""}`} aria-live="polite">
      ...
      {podium.length ? <div className="quest-podium">{podium.map((group) => <div key={group.groupName} className={`place-${group.rank}`}><span>{group.rank === 1 ? "✦" : `#${group.rank}`}</span><strong>{group.groupName}</strong><small>{group.averageEnergy}/{OPTICS_QUEST_MAX_ENERGY}</small></div>)}</div> : <div className="quest-no-rank"><span>✦</span><p>Nhóm cần ít nhất 75% thành viên hoàn thành để được xếp hạng.</p></div>}
      <div className="quest-group-grid">{(snapshot?.groups ?? []).map((group) => <article key={group.groupName} className={`quest-group-card ${group.eligible ? "eligible" : "waiting"}`}><header><div><span>{group.rank ? `#${group.rank}` : "—"}</span><strong>{group.groupName}</strong></div><b>{group.averageEnergy}<small>/{OPTICS_QUEST_MAX_ENERGY}</small></b></header><div className="quest-group-progress"><i><span style={{ width: `${group.completionRate * 100}%` }}/></i><small>{group.finishedCount}/{group.participantCount} xong</small></div><div className="quest-member-dots">{playersByGroup.get(group.groupName)?.map((player) => <span key={player.studentNumber} className={player.finished ? "finished" : player.progress ? "playing" : "ready"} title={`STT ${formatStudentNumber(player.studentNumber)} · ${player.progress}/6 · ${player.energy} NL`}>{formatStudentNumber(player.studentNumber)}</span>)}</div></article>)}</div>
      {!presentation ? <details className="quest-player-details"><summary>Chi tiết học sinh ({snapshot?.readyCount ?? 0})</summary><div className="table-scroll"><table><thead><tr><th>STT</th><th>Nhóm</th><th>Tiến độ</th><th>Năng lượng</th><th>Trạng thái</th></tr></thead><tbody>{(snapshot?.players ?? []).map((player) => <tr key={player.studentNumber}><td>{formatStudentNumber(player.studentNumber)}</td><td>{player.groupName}</td><td>{player.progress}/6</td><td>{player.energy}/{OPTICS_QUEST_MAX_ENERGY}</td><td>{player.finished ? "Hoàn thành" : "Đang làm"}</td></tr>)}</tbody></table></div></details> : null}
