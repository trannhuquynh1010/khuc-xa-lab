...
import { OPTICS_QUEST_MAX_ENERGY, OPTICS_QUEST_QUESTION_COUNT, type OpticsQuestSnapshot } from "@/lib/optics-quest";
...
export default function OpticsQuestDashboard({ className, schoolYear, presentation = false }: { className: string; schoolYear: string; presentation?: boolean }) {
  ...
  return (
    <section className={`optics-quest-dashboard ${presentation ? "presentation" : ""}`} aria-live="polite">
      ...
      {!presentation ? <details className="quest-player-details"><summary>Chi tiết học sinh ({snapshot?.readyCount ?? 0})</summary><div className="table-scroll"><table><thead><tr><th>STT</th><th>Nhóm</th><th>Tiến độ</th><th>Năng lượng</th><th>Trạng thái</th></tr></thead><tbody>{(snapshot?.players ?? []).map((player) => <tr key={player.studentNumber}><td>{formatStudentNumber(player.studentNumber)}</td><td>{player.groupName}</td><td>{player.progress}/{OPTICS_QUEST_QUESTION_COUNT} câu</td><td>{player.energy}/{OPTICS_QUEST_MAX_ENERGY}</td><td>{player.finished ? "Hoàn thành" : "Đang làm"}</td></tr>)}</tbody></table></div></details> : null}
