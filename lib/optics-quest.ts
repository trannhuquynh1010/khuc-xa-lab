...
const stations: OpticsQuestQuestion[][] = [
  [
    ...
    { id: "r-snell-sine", station: 1, stationLabel: "Cổng khúc xạ", title: "Khóa số Snell", prompt: "Tia truyền từ không khí vào thủy tinh có n = 1,50 và sin i = 0,75. Dùng n = sin i / sin r, tính sin r.", kind: "number", answer: "0.5", tolerance: 0.01, explanation: "sin r = 0,75/1,50 = 0,50.", visual: "refraction" },
    { id: "r-speed", station: 1, stationLabel: "Cổng khúc xạ", title: "Tốc độ trong thủy tinh", prompt: "Chiết suất của thủy tinh là 1,50. Lấy tốc độ ánh sáng trong chân không c = 300 000 km/s. Tính v = c/n.", kind: "number", answer: "200000", unit: "km/s", tolerance: 1, explanation: "v = 300 000/1,50 = 200 000 km/s.", visual: "refraction" },
  ],
  [
    ...
    { id: "a-depth", station: 2, stationLabel: "Mắt nhìn dưới nước", title: "Độ sâu nhìn thấy", prompt: "Một con cá ở độ sâu thật 1,20 m. Nhìn gần vuông góc từ trên mặt nước, lấy nₙước = 4/3 và dùng dảnh ≈ dthật/n. Ảnh con cá ở độ sâu bao nhiêu?", kind: "number", answer: "0.9", unit: "m", tolerance: 0.01, explanation: "dảnh ≈ 1,20 ÷ (4/3) = 0,90 m.", visual: "apparent" },
    { id: "a-real-depth", station: 2, stationLabel: "Mắt nhìn dưới nước", title: "Tìm vị trí thật", prompt: "Một vật dưới nước có ảnh nhìn thấy ở độ sâu 0,75 m. Lấy nₙước = 4/3 và dthật ≈ n·dảnh. Tính độ sâu thật.", kind: "number", answer: "1", unit: "m", tolerance: 0.01, explanation: "dthật ≈ (4/3) × 0,75 = 1,00 m.", visual: "apparent" },
  ],
  [
    ...
    { id: "t-critical-sine", station: 3, stationLabel: "Mê cung phản xạ", title: "Mã góc giới hạn", prompt: "Tia đi từ thủy tinh n₁ = 1,50 ra không khí n₂ = 1,00. Dùng sin i₍gh₎ = n₂/n₁, tính sin i₍gh₎ (làm tròn hai chữ số thập phân).", kind: "number", answer: "0.67", tolerance: 0.01, explanation: "sin i₍gh₎ = 1,00/1,50 ≈ 0,67.", visual: "tir" },
    { id: "t-critical-diamond", station: 3, stationLabel: "Mê cung phản xạ", title: "Kim cương và góc giới hạn", prompt: "Kim cương có n₁ = 2,40, không khí có n₂ = 1,00. Dùng sin i₍gh₎ = n₂/n₁, tính sin i₍gh₎ (làm tròn hai chữ số thập phân).", kind: "number", answer: "0.42", tolerance: 0.01, explanation: "sin i₍gh₎ = 1,00/2,40 ≈ 0,42; giá trị nhỏ nên phản xạ toàn phần dễ xảy ra.", visual: "tir" },
  ],
  [
    ...
    { id: "p-color-speed", station: 4, stationLabel: "Mật mã lăng kính", title: "So tốc độ hai màu", prompt: "Trong lăng kính: nđỏ = 1,50 và ntím = 1,60. Lấy c = 300 000 km/s. Tính độ chênh tốc độ vđỏ − vtím.", kind: "number", answer: "12500", unit: "km/s", tolerance: 1, explanation: "vđỏ = 200 000 km/s; vtím = 187 500 km/s; chênh lệch 12 500 km/s.", visual: "prism" },
  ],
  [
  ...
  [
    ...
    { id: "b-critical-code", station: 6, stationLabel: "Boss hải đăng", title: "Mở khóa cửa cuối", prompt: "Tia đi từ môi trường n₁ = 1,60 ra không khí n₂ = 1,00. Dùng sin i₍gh₎ = n₂/n₁. Tính sin i₍gh₎.", kind: "number", answer: "0.625", tolerance: 0.005, explanation: "sin i₍gh₎ = 1,00/1,60 = 0,625.", visual: "boss" },
  ],
];
...
export function createEmptyOpticsQuestAnswers(round: number): OpticsQuestAnswers {
  return { round, groupName: "", questionIds: [], responses: {}, clearedQuestionIds: [], attemptCounts: {} };
}
